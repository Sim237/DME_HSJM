...
class MajorController {
    ...
    public function dashboard() {
        ...
        // ── Patients en attente d'installation ────────────────────────────────
        $stmt = $this->db->prepare("
            SELECT p.*, c.id AS consult_id, u.nom AS medecin_nom, u.prenom AS medecin_prenom
            FROM patients p
        ...
        ");
        $stmt->execute([$serviceId]);
        $patients_a_installer = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // ── Lits libres du service ────────────────────────────────────────────
        $stmt = $this->db->prepare("
            SELECT l.id, l.nom_lit, c.nom_chambre, c.id AS chambre_id
            FROM lits l
            JOIN chambres c ON l.chambre_id = c.id
            WHERE c.service_id = ? AND l.statut = 'DISPONIBLE'
            ORDER BY c.nom_chambre, l.nom_lit
        ");
        $stmt->execute([$serviceId]);
        $lits_libres = $stmt->fetchAll(PDO::FETCH_ASSOC);
