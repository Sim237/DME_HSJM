...
class PortailPatientController {
    ...
    public function rdv() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $sql = "INSERT INTO agenda_medical (patient_id, medecin_id, date_debut, date_fin, motif, statut, type_rdv)
                    VALUES (?, ?, ?, DATE_ADD(?, INTERVAL 30 MINUTE), ?, 'EN_ATTENTE', 'CONSULTATION')";
            $stmt = $this->db->prepare($sql);
            $result = $stmt->execute([
                $patientId,
                $_POST['medecin_id'],
    ...
    private function getRappelsDuJour($patientId) {
        ...
        $sql = "SELECT r.motif as date_rappel, r.date_debut, u.nom as medecin_nom
                FROM agenda_medical r
