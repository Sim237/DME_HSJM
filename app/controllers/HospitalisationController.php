...
class HospitalisationController {
    ...
    public function index() {
        ...
        $stmt = $this->db->prepare("
            SELECT p.*, h.*, s.nom as service_nom, l.numero as lit_numero
            FROM patients p
    ...
    public function administrerTraitement() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $stmt = $this->db->prepare("
                INSERT INTO administrations_medicaments
    ...
    public function ajouterConstantes() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $stmt = $this->db->prepare("
                ...
                (patient_id, temperature, tension_systolique, tension_diastolique, frequence_cardiaque,
                 frequence_respiratoire, saturation_o2, glycemie, poids, taille, date_mesure, infirmier_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)
            ");
    ...
    public function planifierSoin() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $stmt = $this->db->prepare("
                INSERT INTO soins_planifies
                (patient_id, type_soin, description, heure_prevue, statut, prescripteur_id)
                VALUES (?, ?, ?, ?, 'planifie', ?)
            ");
    ...
    private function getTraitementsActifs($patient_id) {
        $stmt = $this->db->prepare("
            SELECT ph.*, m.nom as medicament_nom, m.forme, m.dosage
    ...
    private function getPrescriptionsHospitalisation($patient_id) {
        $stmt = $this->db->prepare("
            SELECT ph.*, m.nom as medicament_nom, u.nom as medecin_nom
            FROM prescriptions_hospitalisation ph
            JOIN medicaments m ON ph.medicament_id = m.id
...
public function planifierSoins($patient_id) {
    ...
    $stmt = $db->prepare("
        SELECT h.*, s.nom_service, l.nom_lit, c.nom_chambre
...
public function checklist($plan_id) {
    ...
    $stmtPatient = $db->prepare("
        SELECT p.nom, p.prenom, sp.date_plan
...
public function observationsEvolution($patient_id) {
    ...
    // Récupérer l'historique des observations
    $db = (new Database())->getConnection();
    ...
    // Charger la vue (Vérifiez bien le nom du fichier)
...
public function executerSoins($plan_id) {
    ...
    $stmt = $db->prepare("
        SELECT sd.*, p.nom, p.prenom, p.dossier_numero, sp.id as plan_id
