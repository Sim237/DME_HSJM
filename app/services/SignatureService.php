...
class SignatureService {
    ...
    public function __construct() {
        ...
        $this->ensureNullableSignatureImage();
    }

    private function ensureNullableSignatureImage(): void {
        try {
            $row = $this->db->query("
                SELECT IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS
                WHERE TABLE_SCHEMA = DATABASE()
                  AND TABLE_NAME = 'medecin_signatures'
                  AND COLUMN_NAME = 'signature_image'
            ")->fetch(PDO::FETCH_ASSOC);
            if ($row && $row['IS_NULLABLE'] === 'NO') {
                $this->db->exec("ALTER TABLE medecin_signatures MODIFY COLUMN signature_image LONGTEXT NULL DEFAULT NULL");
            }
    ...
    public function uploadAndResize(array $file, string $type, int $userId): ?string
    {
        ...
        // INSERT with NULL for the other column so it doesn't overwrite existing data
        $this->db->prepare("
            INSERT INTO medecin_signatures (medecin_id, signature_image, cachet_image, is_active)
            VALUES (?, ?, ?, 1)
            ON DUPLICATE KEY UPDATE {$col} = VALUES({$col}), updated_at = NOW()
        ")->execute([
            $userId,
            $type === 'signature' ? $b64 : null,
            $type === 'cachet'    ? $b64 : null,
        ]);
    ...
    {
        // Normaliser vers les valeurs ENUM autorisées
        // Normaliser vers les valeurs ENUM de la table documents_signes
        $map = [
            'CRH'               => 'RAPPORT',
            'COMPTE_RENDU'      => 'RAPPORT',
            'COMPTE_RENDU_HOSP' => 'RAPPORT',
            'BILAN'             => 'RAPPORT',
            'IMAGERIE'          => 'RAPPORT',
        ];
        $document_type = $map[$document_type] ?? $document_type;
        $allowed = ['ORDONNANCE','CERTIFICAT','RAPPORT','AUTRE'];
